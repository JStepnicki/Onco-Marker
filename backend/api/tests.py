...
from api.views import classify_sample

class TestClassifySample(TestCase):
    @classmethod
    def setUpTestData(cls):
        # Utwórz przykładowe próbki do testów
    ...
    def test_classify_sample_existing_sample(self, mock_get):
        # Skonfiguruj metodę mock_get, aby zwracała przykładową próbkę
        mock_get.return_value = self.sample1

        # Wywołaj funkcję classify_sample
        y_pred, stage_pred = classify_sample(self.sample1.id, "pancreas")

        # Sprawdź, czy funkcja zwraca poprawne wyniki
        self.assertEqual(y_pred, "1")
        self.assertIsNone(stage_pred)

    @patch('api.views.CancerSample.objects.get')
    def test_classify_sample_missing_sample(self, mock_get):
        # Skonfiguruj metodę mock_get, aby zgłaszała wyjątek CancerSample.DoesNotExist
        mock_get.side_effect = CancerSample.DoesNotExist

        # Wywołaj funkcję classify_sample
        result = classify_sample(999, "pancreas")

        # Sprawdź, czy funkcja zwraca komunikat o błędzie
        self.assertIn("error", result)

    @patch('api.views.CancerSample.objects.get')
    def test_classify_sample_diagnosis_3(self, mock_get):
        # Skonfiguruj metodę mock_get, aby zwracała próbkę z diagnozą "3"
        mock_get.return_value = self.sample3

        # Wywołaj funkcję classify_sample
        y_pred, stage_pred = classify_sample(self.sample3.id, "pancreas")
